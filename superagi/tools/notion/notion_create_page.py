...
from superagi.helper.notion_helper import NotionHelper
from superagi.helper.prompt_reader import PromptReader

class NotionCreatePageSchema(BaseModel):
    content:  list = Field( 
        ...,
        description=PromptReader.read_tools_schema_description(__file__,"notion_create_page.txt"),
    )
    title: str = Field(
        ...,
...
class NotionCreatePageTool(BaseTool):
    ...
    def _execute(self, content:list, title:str, tags=None) -> str:
        """
        ...
        try:
            ...
            response=notion_helper.create_page(content,title,Notion_database_id,tags)
